...
import { validate as peepee } from "@atlc/hibp";
...
export const giveTokenToExistingUser = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (userFound && compareHash(password, userFound.password)) {
            ...
            const token = jwt.sign({ username: userFound.authorname, userid: userFound.id, email: userFound.email, role: `guest` }, JWT_CONFIG.jwtSecretKey, { expiresIn: `10d` });
